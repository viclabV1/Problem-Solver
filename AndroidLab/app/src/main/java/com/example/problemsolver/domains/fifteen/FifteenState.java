...
public class FifteenState extends State {
     ...
     * for this state
     */
    public FifteenState (FifteenState state, Location loc1, Location loc2) {
    ...
    public boolean equals(Object o) {
        if (o == null) return false;
        if (o.getClass() != FifteenState.class) return false;
        FifteenState other = (FifteenState) o;
    ...
    public int hashCode() {
        int hash = 7;
        hash = 47 * hash + Arrays.deepHashCode(this.tiles);
        return hash;
    }
...
//        return tilesOutOfPlace(this, goal);
    ...
    public static int tilesOutOfPlace(State current, State goal) {
        int count = 0;
        FifteenState currentState = (FifteenState)current;
        FifteenState goalState = (FifteenState)goal;
        for(int i = 1; i < 16; i++){
            if(!currentState.getLocation(i).equals(goalState.getLocation(i))) count++;
	...
	FifteenState currentState = (FifteenState)current;
        FifteenState goalState = (FifteenState)goal;
        for(int i = 1; i < 16; i++){
            int rowDifference = Math.abs(currentState.getLocation(i).getRow()-goalState.getLocation(i).getRow());
            int colDifference = Math.abs(currentState.getLocation(i).getColumn()-goalState.getLocation(i).getColumn());
            sum = sum + rowDifference + colDifference;
        }
        return sum;
    }
